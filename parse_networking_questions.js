...
function parseQuestions(filename) {
    ...
    const saveCurrent = () => {
        if (currentNum !== null && currentQuestion) {
            ...
            if (!explanation) {
                explanation = "A foundational concept in Networking & Protocols.";
            }
    ...
    // Filter out some final trailing lines which might be study tips, etc
    questions = questions.filter(q => q.topic && q.question.length > 2);

    console.log(`Parsed ${questions.length} questions.`);
    
    const output = `import { Question } from "../services/quizService";\n\nexport const NETWORKING_NEW_QUESTIONS: Question[] = ${JSON.stringify(questions, null, 2)};\n`;
    fs.writeFileSync('parsed_networking_questions.ts', output, 'utf-8');
}

parseQuestions('questions_raw_networking.txt');
